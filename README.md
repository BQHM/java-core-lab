...
| `ThreadPoolJmhBenchmark` | 线程池参数调优 & JMH 基准测试 | 实测：IO 密集场景核心线程数 = CPU×2 效率最优；并发≤核心线程数时增线程无收益，线程过多切换频繁性能反降 | ✅ 已完成 |
| `DeadLockDemo` | jstack+Arthas死锁排查实战 | 实测jstack定位线程锁持有/等待关系，Arthas thread -b一键定位死锁元凶；验证死锁4个必要条件，通过统一锁获取顺序打破循环等待完成修复 | ✅ 已完成 |
| `HighCpuDemo` | 线上CPU飙高问题排查实战 | 实测传统CPU排查四步法核心逻辑，Arthas thread -n一键定位高CPU占用线程与问题代码行；验证密集计算/死循环导致CPU飙高的根因 | ✅ 已完成 |
